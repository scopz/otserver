...
bool ConjureSpell::ConjureFood(const ConjureSpell* spell, Creature* creature, const std::string& param)
{
	...
	uint32_t foodType[9] = {
		...
		ITEM_COCONUT,        // 216
		ITEM_BREAD,          // 120
		ITEM_BROWN_MUSHROOM, // 264
		ITEM_BROWN_MUSHROOM, // 264
	};

	bool result = internalConjureItem(player, foodType[random_range(0, 8)], random_range(1, 3)) == RET_NOERROR;
	...
	const PositionEx& posTo, bool extendedUse, uint32_t creatureId)
{
	...
	if(result){
		Spell::postCastSpell(player);
		if (item && g_config.getNumber(ConfigManager::REMOVE_RUNE_CHARGES)){
