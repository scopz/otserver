	...
	if(output){
		...
		switch(m_state) {
			case ENCRYPTION_NO_SET:
				success = actionEncryptConnection(recvbyte, outputPool, output);
				break;

			case NO_LOGGED_IN:
				success = actionLogIn(recvbyte, outputPool, output);
				break;

			case LOGGED_IN:
				success = actionLoggedIn(recvbyte, outputPool, output);
				break;
		...
		case AP_MSG_COMMAND:
		{
			...
			case CMD_BROADCAST:
			{
				...
				g_dispatcher.addTask(
					createTask(std::bind(&Game::anonymousBroadcastMessage, &g_game, MSG_STATUS_WARNING, message)));
			...
			{
				g_dispatcher.addTask(
					createTask(std::bind(&ProtocolAdmin::adminCommandOpenServer, this)));
			...
			{
				g_dispatcher.addTask(
					createTask(std::bind(&ProtocolAdmin::adminCommandCloseServer, this)));
			...
			{
				g_dispatcher.addTask(
					createTask(std::bind(&ProtocolAdmin::adminCommandPayHouses, this)));
			...
			{
				g_dispatcher.addTask(
					createTask(std::bind(&ProtocolAdmin::adminCommandShutdownServer, this)));
				return;
				break;
			}
			case CMD_SEND_MAIL:
			{
				...
				g_dispatcher.addTask(
					createTask(std::bind(&ProtocolAdmin::adminCommandSendMail, this, xmlData)));
				break;
			}
			case CMD_KICK:
			{
				...
				g_dispatcher.addTask(
					createTask(std::bind(&ProtocolAdmin::adminCommandKickPlayer, this, name)));
				break;
			}
			...
			{
				g_dispatcher.addTask(
					createTask(std::bind(&ProtocolAdmin::adminCommandSaveServer, this, command == CMD_SHALLOW_SAVE_SERVER)));
			...
			{
				g_dispatcher.addTask(
					createTask(std::bind(&ProtocolAdmin::adminCommandRelationalSaveServer, this)));
...
bool ProtocolAdmin::actionEncryptConnection(uint8_t recvbyte, OutputMessagePool* outputPool, OutputMessage_ptr output)
{
	if(g_adminConfig->requireEncryption()){
		...
		m_state = NO_LOGGED_IN;
		return actionLogIn(recvbyte, outputPool, output);
	}
	return true;
}

bool ProtocolAdmin::actionLogIn(uint8_t recvbyte, OutputMessagePool* outputPool, OutputMessage_ptr output)
{
	if(g_adminConfig->requireLogin()){
		...
		m_state = LOGGED_IN;
		return actionLoggedIn(recvbyte, outputPool, output);
	}
	return true;
}

bool ProtocolAdmin::actionLoggedIn(uint8_t recvbyte, OutputMessagePool* outputPool, OutputMessage_ptr output)
{
