...
function setFirstItems(cid)
	if isNoneVocation(cid) then

		if (getPlayerSex(cid) == 0) then
			doPlayerAddItem(cid, 2651, 1) -- coat
		else
...
function isReborn(cid)
	if(isPlayer(cid) == false) then
		debugPrint("isReborn: Player not found.")
		return false
	end
	...
	local reb = getRebirthVocation(voc)
	return reb == 0
end
...
function doPlayerGiveItem(cid, itemid, count, charges)
	local hasCharges = (isItemFluidContainer(itemid) == true)
