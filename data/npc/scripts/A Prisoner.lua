...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'math') and queststate1 == 1 then
	    npcHandler:playerSay(cid, 'My surreal numbers are based on astonishing facts. Are you interested in learning the secret of mathemagics?')
	    cidData.state = 1

	elseif msgcontains(msg, 'yes') and cidData.state == 1 then
        npcHandler:playerSay(cid, 'But first tell me your favourite colour please!')
        cidData.state = 2

	elseif msgcontains(msg, 'green') and cidData.state == 2 then
        npcHandler:playerSay(cid, 'Very interesting. So are you ready to proceed in you lesson in mathemagics?')
		doPlayerRemoveMoney(cid,1000)
		cidData.state = 3

	elseif msgcontains(msg, 'yes') and cidData.state == 3 then
        npcHandler:playerSay(cid, 'So know that everthing is based on the simple fact that 1 + 1 = 2!')
		setPlayerStorageValue(cid,6668,1)
        cidData.state = 4

	elseif msgcontains(msg, 'bye') and (cidData.state >= 1 and cidData.state <= 4) then
		npcHandler:playerSay(cid, 'Next time we should talk about my surreal numbers.')
		cidData.state = 0

	elseif msgcontains(msg, 'key') then
		npcHandler:playerSay(cid, 'Do you want a key to the mad mage room?')
		cidData.state = 5

	elseif msgcontains(msg, 'yes') and cidData.state == 5 then
		npcHandler:playerSay(cid, 'Here you are a special key.')
