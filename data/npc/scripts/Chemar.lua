...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'flying') and npcHandler:hasFocus(cid) then
		npcHandler:playerSay(cid, "Do you want to buy a flying carpet for 5000 platinum coins?", 1)
		cidData.state = 7358
	elseif cidData.state == 7358 and msgcontains(msg, 'yes') and npcHandler:hasFocus(cid) then
		if getPlayerItemCount(cid,2160) >= 500 then
			npcHandler:playerSay(cid, "Oh, I am sorry, but you have no pilot licence.", 1)
			cidData.state = 0
		else
			npcHandler:playerSay(cid, "You don't own enough worldly wealth to afford this item.", 1)
			cidData.state = 0
		end

	elseif cidData.state == 7358 and msgcontains(msg, 'no') and npcHandler:hasFocus(cid) then
		npcHandler:playerSay(cid, "Maybe another day then, my friend.", 1)
		cidData.state = 0

	elseif msgcontains(msg, 'mail') and npcHandler:hasFocus(cid) then
		npcHandler:playerSay(cid, "Our mail system is unique! And everyone can use it. Do you want to know more about it?", 1)
		cidData.state = 4278
	elseif cidData.state == 4278 and msgcontains(msg, 'yes') and npcHandler:hasFocus(cid) then
		npcHandler:playerSay(cid, "The Darashian Airmail System enables you to send and receive letters and parcels. You can buy them here if you want.", 1)
		cidData.state = 0

	elseif msgcontains(msg, 'parcel') and npcHandler:hasFocus(cid) then
		npcHandler:playerSay(cid, "Do you want to buy a parcel for 15 gold?", 1)
		cidData.state = 3532
	elseif cidData.state == 3532 and msgcontains(msg, 'yes') and npcHandler:hasFocus(cid) then
		if doPlayerRemoveMoney(cid, 15) == true then
			...
			npcHandler:playerSay(cid, "Oh, you have not enough gold to buy a parcel.", 1)
			cidData.state = 0
		end
	elseif msgcontains(msg, 'fly') and npcHandler:hasFocus(cid) then
		npcHandler:playerSay(cid, "I can fly you to Femor Hills or Edron if you like. Where do you want to go?", 1)
		cidData.state = 0
	end
