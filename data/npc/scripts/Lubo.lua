...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'crunor\'s cottage') and queststate == 1 then
		npcHandler:playerSay(cid, 'Ah yes, I remember my grandfather talking about that name. This house used to be an inn a long time ago. My family bought it from some of these flower')
		cidData.state = 1
	elseif msgcontains(msg, 'flower guys') and cidData.state == 1 then
        npcHandler:playerSay(cid, 'Oh, I mean druids of course. They sold the cottage to my family after some of them died in an accident or something like that.')
        cidData.state = 2
	elseif msgcontains(msg, 'accident') and cidData.state == 2 then
        npcHandler:playerSay(cid, 'As far as I can remember the story, a pet escaped its stable behind the inn. It got somehow involved with powerfull magic at a ritual and was transformed in some way.')
        cidData.state = 3
	elseif msgcontains(msg, 'stable') and cidData.state == 3 then
        npcHandler:playerSay(cid, 'My grandpa told me, in the old days there were some behind this cottage. Nothing big though, just small ones, for chicken or rabbits.')
