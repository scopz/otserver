...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'measurements') and getPlayerStorageValue(cid,234) > 0 and getPlayerStorageValue(cid,240) < 1 then
		npcHandler:playerSay(cid, 'My measurements? Listen, lets make that a bit more exciting ... No, no, not what you think! I mean let\'s gamble. I will roll a dice. If I roll a 6 you win and I\'ll tell you what you need to know, else I win and get 5 gold. Deal?')
		cidData.amount = math.random(1,6)
		cidData.state = 5

	elseif cidData.state == 5 and msgcontains(msg, 'no') then
		npcHandler:playerSay(cid, 'This way you\'ll never get my measurements.')
		cidData.state = 0

	elseif cidData.state == 5 and getPlayerMoney(cid) >= 5 and cidData.amount == 6 then
		npcHandler:playerSay(cid, 'Ok, here we go ... 6! You have won! How lucky you are! So listen ...<tells you what you need to know>')
		setPlayerStorageValue(cid,240,1)
		setPlayerStorageValue(cid,234,getPlayerStoraValue(cid,234)+1)
		cidData.state = 0

	elseif cidData.state == 5 and getPlayerMoney(cid) >= 5 then
		npcHandler:playerSay(cid, 'Ok, and its ... 5! You have lost. He he. Another game?')
		doPlayerRemoveMoney(cid, 5)
		cidData.state = 6

	elseif cidData.state == 5 and getPlayerMoney(cid) < 5 then
		npcHandler:playerSay(cid, 'I am sorry, but you don\'t have so much money.')
		cidData.state = 0

	elseif cidData.state == 6 and msgcontains(msg, 'yes') then
		npcHandler:playerSay(cid, 'Ok, no weights in the dice, no dirty tricks, are you ready?')
		cidData.amount2 = math.random(1,6)
		cidData.state = 7

	elseif cidData.state == 6 and msgcontains(msg, 'no') then
		npcHandler:playerSay(cid, 'This way you\'ll never get my measurements.')
		cidData.state = 0

	elseif cidData.state == 7 and getPlayerMoney(cid) >= 5 and cidData.amount2 == 6 then
		npcHandler:playerSay(cid, 'Ok, here we go ... 6! You have won! How lucky you are! So listen ...<tells you what you need to know>')
		setPlayerStorageValue(cid,240,1)
		setPlayerStorageValue(cid,234,getPlayerStorageValue(cid,234)+1)
		cidData.state = 0

	elseif cidData.state == 7 and msgcontains(msg, 'no') then
		npcHandler:playerSay(cid, 'This way you\'ll never get my measurements.')
		cidData.state = 0

	elseif cidData.state == 7 and getPlayerMoney(cid) >= 5 then
		npcHandler:playerSay(cid, 'Ok, and its ... 5! You have lost. He he. Another game?')
		doPlayerRemoveMoney(cid, 5)
		cidData.state = 6
