...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'book') or msgcontains(msg, 'notebook') then
		npcHandler:playerSay(cid, 'Do you bring me my notebook?')
		cidData.state = 1

	elseif msgcontains(msg, 'yes') and cidData.state == 1 and getPlayerItemCount(cid,1955) >= 1 then
		npcHandler:playerSay(cid, 'Excellent. Here, take this short sword, that might serve you well.')
		doPlayerAddItem(cid, 2406)
		doPlayerRemoveItem(cid,1955, 1)
		cidData.state = 0
	elseif msgcontains(msg, 'yes') and cidData.state == 1 and getPlayerItemCount(cid,1955) == 0 then
	...
	elseif msgcontains(msg, 'orcish') or msgcontains(msg, 'language') or msgcontains(msg, 'prisoner') then
		npcHandler:playerSay(cid, 'I speak some orcish words, not much though, just \'yes\' and \'no\' and such basic.')
		cidData.state = 2

	elseif msgcontains(msg, 'yes') and cidData.state == 2 then
		npcHandler:playerSay(cid, 'It\'s \'mok\' in orcish. I help you more about that if you have some food.')
	elseif msgcontains(msg, 'no') and cidData.state == 2 then
		npcHandler:playerSay(cid, 'In orcish that\'s \'burp\'. I help you more about that if you have some food.')

	elseif msgcontains(msg, 'food') then
		npcHandler:playerSay(cid, 'My favorite dish is salmon. Oh please, bring me some of it.')

	elseif msgcontains(msg, 'salmon') then
		npcHandler:playerSay(cid, 'Yeah! If you give me some salmon I will tell you more about the orcish language.')
		cidData.state = 3

	elseif msgcontains(msg, 'yes') and cidData.state == 3 and getPlayerItemCount(cid,2668) >= 1 then
		npcHandler:playerSay(cid, 'Thank you. Orcs call arrows \'pixo\'.')
		doPlayerRemoveItem(cid,2668,1)
	elseif msgcontains(msg, 'yes') and cidData.state == 3 and getPlayerItemCount(cid,2668) == 0 then
		npcHandler:playerSay(cid, 'You don\'t have one!')
		cidData.state = 0

	elseif msgcontains(msg, 'bye') and (cidData.state >= 1 and cidData.state <= 3) then
		npcHandler:playerSay(cid, 'See you later.')
		cidData.state = 0
	end
