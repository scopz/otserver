...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'spark') or msgcontains(msg, 'phoenix') then
		npcHandler:playerSay(cid, "The spark of the phoenix is given by me and by the great geomancer of the local earthtemple. Do you wish to receive my part of blessing of the phoenix for 10.000 gold?", 1)
		cidData.state = 1394
		...
		if getPlayerStorageValue(cid, 1339) >= 1 then
			if doPlayerRemoveMoney(cid, 10000) == true then
				if doPlayerAddBless(cid, 5) == true then
					npcHandler:playerSay(cid, "So receive the mark of the flame and be blessed by the phoenix, pilgrim.", 1)
			...
			npcHandler:playerSay(cid, "You need the blessing of the great geomancer first.", 1)
			cidData.state = 0
		end
