...
function creatureSayCallback(cid, type, msg)
	...
	local questValue = getPlayerStorageValue(cid, 233)

	-- The Postman Missions Quest
	if msgcontains(msg, 'uniform') and questValue == 0 then
		npcHandler:playerSay(cid, 'I don\'t get it, what uniforms you are talking about.')

	elseif msgcontains(msg, 'uniform') and questValue == 1 then
		npcHandler:playerSay(cid, 'A new uniform for the post officers? I am sorry but my dog ate the last dress pattern we used. You need to supply us with a new dress pattern.')

	elseif msgcontains(msg, 'dress pattern') and questValue == 1 then
		npcHandler:playerSay(cid, 'It was ... wonderous beyond wildest imaginations! I have no clue where Kevin Postner got it from. Better ask him.')
		setPlayerStorageValue(cid, 233, 2)

	elseif msgcontains(msg, 'dress pattern') and questValue > 1 and questValue < 9 then
		npcHandler:playerSay(cid, 'I already told you to ask your boss about that issue.')

	elseif msgcontains(msg, 'dress pattern') and questValue == 9 then
		npcHandler:playerSay(cid, 'By the gods of fashion! Didn\'t it do that I fed the last dress pattern to my poor dog? Will this mocking of all which is taste and fashion never stop?? Ok, ok, you will get those ugly, stinking uniforms and now get lost, fashion terrorist.')
