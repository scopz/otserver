...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'rat') then
		npcHandler:playerSay(cid, "I'll give you 2 gold for per dead rat. Do you accept?", 1)
		cidData.state = 2
	...
	elseif cidData.state == 2 and msgcontains(msg, 'yes') then
		...
		if AMOUNTRAT >= 1 then
			if doPlayerRemoveItem(cid, 2813, AMOUNTRAT) == true then
				...
				npcHandler:playerSay(cid, "Deal. By the way: If you'd like to hunt something bigger, check the cellar of the stables to the north. Some adventurer used to store his loot under a loose board beneath a barrel. He might have forgotten something when he left the isle.", 1)
			end
		else
			npcHandler:playerSay(cid, "Sorry, you do not have any fresh one.", 1)
		end
		cidData.state = 0
	...
	elseif msgcontains(msg, 'rabbit') then
		npcHandler:playerSay(cid, "I'll give you 2 gold for per dead rabbit. Do you accept?", 1)
		cidData.state = 3
	...
	elseif msgcontains(msg, 'wolf') or msgcontains(msg, 'wolves') then
		npcHandler:playerSay(cid, "I'll give you 5 gold for per dead worlf. Do you accept?", 1)
		cidData.state = 4
