...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'yes') and cidData.state == 3 then
		npcHandler:playerSay(cid, 'So what does trouble your soul, pilgrim?')
		cidData.state = 4
	...
	elseif msgcontains(msg, '') and cidData.state == 4 then
		npcHandler:playerSay(cid, 'Meditate on that and try harder to improve soul.')
		cidData.state = 0

	elseif msgcontains(msg, 'yes') and getPlayerMoney(cid) >= 15 and cidData.state == 1 then
		doPlayerRemoveMoney(cid, 15)
		npcHandler:playerSay(cid, 'May Daraman guide your quest for ascension.')
		cidData.state = 0
