...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'gambl') or msgcontains(msg, 'game') or msgcontains(msg, 'dice') then
		npcHandler:playerSay(cid, "So you care for a civilized game of dice?", 1)
		cidData.state = 0

	elseif cidData.state == 0 and msgcontains(msg, 'yes') then
		npcHandler:playerSay(cid, "I will roll a dice. If it shows 6, you will get five times your bet. How much do you want to bet?", 1)
		cidData.state = 1
		...
		if amount >= 1 and amount <= 99 then
			if doPlayerRemoveMoney(cid, amount) == true then
				if CHANCE == 6 then
					...
					npcHandler:playerSay(cid, "Ok, here we go ... 6! You have won ".. amount*5 ..", congratulations. One more game?", 1)
					cidData.state = 0
				else
					npcHandler:playerSay(cid, "Ok, here we go ... ".. CHANCE .."! You have lost. Bad luck. One more game?", 1)
					cidData.state = 0
				end
			else
				npcHandler:playerSay(cid, "I am sorry, but you don't have so much money. How much do you want to bet?", 1)
				cidData.state = 1
			end
		else
			npcHandler:playerSay(cid, "I am sorry, but I accept only bets between 1 and 99 gold. I don't want to ruin you after all. How much do you want to bet?", 1)
			cidData.state = 1
		end
	end
	return true
